...
KRD and coins on KRYZA Diamond are supported by the Ledger Nano S hardware wallet. The following describes how to use Ledger Nano S to connect to TC
...
!!! Tip
		The existing Ethereum App is compatible with KRYZA Diamond
...
2) Within the "Manager" pane, type in "Ethereum" in the search field. Locate "Ethereum", then click on "Install". (Note: Do not use the "KRYZA Diamond" App)
...
## Enabling contract data for KRC20 

!!! Tip
		 Please ensure your enabled contract data before transfering KRC20 tokens 
...
KRYZA Diamond is compatible with MetaMask. Users can use MetaMask as a bridge between Ledger Nano S and KRYZA Diamond to make transactions to the KRYZA Diamond testnet.

1) Connect [MetaMask](./metamask.md) to KRD Testnet by following this guide to add a custom network

 * Testnet
    * [RPC URLs](../developer/rpc.md)
    * ChainID: 0xBED1 (48849 in decimal)
    * Symbol: KRD
    * [Block Explorer](https://testnet-explorer.kryzascan.com)

 * Mainnet
    * [RPC URLs](../developer/rpc.md)
    * ChainID: 0x17C53 (97363 in decimal)
    * Symbol: KRD
...
1. Fail to tranfer KRC20 tokens
