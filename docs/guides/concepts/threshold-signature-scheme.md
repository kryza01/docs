...
Combining TSS feature  with KRYZA Diamond client will help users manage their funds in a much safer way. TSS will be offered in an independent binary, but it will have some impact on the existing functions of *eth-cli/eth-cli*.

## Implementation

Many development resources have been poured into implementing TSS, a cryptographic protocol for distributed key generation and signing. TSS is now compatible and reusable for ECDSA-based blockchains, including KRYZA Diamond, Bitcoin, and Ethereum networks. We expect that members of the KRD ecosystem and partner community can integrate this TSS library with their applications, such as wallets and custodians, and further develop this exciting new technology.

With the support of the KRYZA Diamond community, we are happy to share the open-source code athttps://github.com/githubusername/githubrepo/tss-lib.
...
Security checks are routinely and continuously planed for all parts of TSS lib implmentations and future audits  will be reported to KRYZA Diamond community.
...
* **Verification**: the verification algorithm remains as it is in the classical setting. To be compatible with single key signatures, KRYZA Diamond validator nodes can be able to verify the signature with the public key. The transaction will be no different from others.
...
##  Where can I download the KRD TSS CLI?

You can download tss client and KRYZA Diamond Commandline here:
...
| Name       | Type   | Description                                                  | Note                                              |
| ---------- | ------ | ------------------------------------------------------------ | ------------------------------------------------- |
|address_prefix|string |bech32 prefix of address (default "krd")||
...
| Name       | Type   | Description                                                  | Note                                              |
| ---------- | ------ | ------------------------------------------------------------ | ------------------------------------------------- |
|address_prefix|string|prefix of bech32 address|the default value is `krd`|
...
|                            | A                                                            | B                                                            | C    |
| -------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ---- |
| command                    | eth-cli send --amount 1000000:KRD --to tbnb1mh3w2kxmdmnvctt7t5nu7hhz9jnp422edqdw2d --from tss_tss1_vault1 --chain-id KRD-Chain-Ganges --node https://data-seed-pre-0-s1.kryzascan.com:443 --trust-node | eth-cli send --amount 1000000:KRD --to tbnb1mh3w2kxmdmnvctt7t5nu7hhz9jnp422edqdw2d --from tss_tss2_vault1 --chain-id KRD-Chain-Ganges --node https://data-seed-pre-0-s1.kryzascan.com:443 --trust-node | NA   |
