...
1. Compose a data structure. please note `msgs`, `memo`, `source`, `data` are the same as in the above `payload`.
    ...
    - `chain_id`: a string, unique ID for the Chain, it stays the same for most time, but may vary as KRYZA Diamond evolves;
...
The `signature` will be encoded together with transaction message and sent as `payload` to KRYZA Diamond node via RPC or http REST API, as described in the above section.
