...
## Slashing
Slashing is part of the on-chain governance, to ensure the malicious or negative behaviors are punished. KRD slash can be submitted by anyone. The transaction submission requires slash evidence and cost fees but also brings a larger reward when it is successful.

So far there are two slashable cases.

### Double Sign

Anyone can submit a slash request on BC with the evidence of Double Sign of TC
...
1. 1000KRD would be slashed from the **self-delegated** KRD of the validator
2. If the self-delegator’s stake amount on the validator is less than 1000KRD, then the unbonding delegation balance would be slashed if it exists until totally 1000KRD slashed from self-delegator of the validator. However, if all the slashed KRD is less than 1000, all the remaining stake of the self-delegator will be slashed
3. 100 of slashed KRD would allocate to the submitter as a reward
4. The rest of slashed KRD will allocate to the custody addresses of which validators would take part in the next distribution. If no matched validators found, then the rest of slashed KRD will allocate to validators on BC as block fee
5. Set the validator ‘jailed’ with duration of 7 days, and remove it from validator set by an instance KRD validator set update Cross-Chain update
...
1. 50KRD would be slashed from the  **self-delegated** KRD of the validator
2. If the self-delegator’s stake amount on the validator is less than 50KRD, then the unbonding delegation balance would be slashed if it exists until totally 50KRD slashed from self-delegator of the validator. However, if all the slashed KRD is less than 1000, all the remaining stake of the self-delegator will be slashed
3. 10 of slashed KRD would allocate to the validators on BC as block fee
4. The rest of slashed KRD will allocate to the custody addresses of which validators would take part in the next distribution. If no matched validators found, then the rest of slashed KRD will allocate to validators on BC as block fee
5. Set the validator ‘jailed’ with duration of 2 days, and remove it from validator set by an instance KRD validator set update Cross-Chain update
...
When your validator is `unjailed` on shree chain, it must wait for the next UTC 0:00 to join validatorsest again.

### Transaction Validation
* validator address must not be empty
* side chain id exists
* self-delegation of the validator exists and the tokens of it must be greater than the min-self-delegation setting by 20000KRD
...
Please download `eth-cli` binary from [here](https://github.com/githubusername/githubrepo/smart-chain-binary/tree/master/bc/cli)

### Submit KRD evidence

Slashing validators of KRD for the malicious behavior of **double-sign** by submitting evidence consisting of two block headers with same height but signed by one signer

#### Parameters for  slashing nc-submit-evidence

| **parameter name** | **example**                                                  | **comments**                                                 | **required** |
| ------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------ |
| --chan-id          | KRD-Chain-Tigris                                          | the chain id of shree  chain                               | Yes          |
| --from             | bnb19awsmku5ch689lp0rj0c6su7x0n5wxhjm65hdd                   | Name or address of  private key with which to sign           | Yes          |
| --evidence         | [{"difficulty":"0x2","extraData":"0xd98301...},{"difficulty":"0x3","extraData":"0xd64372...}] | Evidence details,  including two KRD block headers with json format | Option       |
...
| --side-chain-id    | TC-XXX                                    | chain-id of the side  chain the validator belongs to         | Yes          |
...
```bash
eth-cli slashing side-unjail --from bnb19awsmku5ch689lp0rj0c6su7x0n5wxhjm65hdd --side-chain-id=KRD --chain-id=test-chain-8d7sJz --home ~/home_cli
```
...
* Mainnet
```bash
eth-cli slashing side-signing-info 0x625448c3f21AB4636bBCef84Baaf8D6cCdE13c3F --side-chain-id=KRD --chain-id=test-chain-8d7sJz --home ~/home_cli
```
...
* Mainnet
```bash
eth-cli slashing side-slash-history 0x625448c3f21AB4636bBCef84Baaf8D6cCdE13c3F --infraction-height 100 --infraction-type DoubleSign --side-chain-id=KRD --chain-id=test-chain-8d7sJz --home ~/home_cli
```
...
* Mainnet
```bash
eth-cli slashing side-slash-histories 0x625448c3f21AB4636bBCef84Baaf8D6cCdE13c3F --infraction-type DoubleSign --side-chain-id=KRD --chain-id=test-chain-8d7sJz --home ~/home_cli
```
...
* Mainnet
```bash
eth-cli slashing side-all-slash-histories --side-chain-id=KRD --chain-id=test-chain-8d7sJz --home ~/home_cli
```
