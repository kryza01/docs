...
- [ ] https://docs.kryzascan.com/walletconnect.html
...
## Wallets with WalletConnect Support for KRYZA Diamond
...
The KRYZA Diamond Web Wallet supports connecting with external wallet providers via the [WalletConnect protocol](https://docs.walletconnect.org/tech-spec).
...
In order for this to work, some modifications to the standard WalletConnect protocol are used in the KRYZA Diamond wallet's implementation.

See the list of wallets which support WalletConnect on KRYZA Diamond [here](../../wallets/walletconnect-support.md)
...
* Typically `sendTransaction` is used with Ethereum transaction parameters in WalletConnect dApp integrations. But in KRYZA Diamond's case, instead of invoking `sendTransaction` in the WalletConnect flow, the new [`sendCustomRequest`](https://docs.walletconnect.org/client-sdk#send-custom-request) call is used instead with a method called `bnb_sign` (see below).
